...
//  RNGine
//
//  Created by Dom S on 9/25/25.
//

import SwiftUI

extension View {
    /// Allows attaching an adaptive bottom sheet (drawer) to any view.
    /// - Parameters:
    /// - isPresented: Binding controlling if the sheet should appear.
    /// - content:  The actual content of the sheet.
...
// MARK: - Adaptive Sheet
/// Main modifier that handles displaying of sheet.
...
private struct CustomBottomSheet<Content: View>: View {
    ...
    private func calculatedOffset(for position: SheetPosition, height: CGFloat) -> CGFloat {
        let compactOffsetPosition: CGFloat = 50
